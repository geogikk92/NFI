...
await run("Сертификати: издаване → профил → проверка → отмяна", async ({ page, ok }) => {
  ...
  // Срокът е 45 s, не 20 s, и това НЕ е замазване на бавнина.
  //
  // Това изпращане прави най-тежкото нещо в сайта: взима номер от общия
  // брояч, генерира PDF с ВГРАДЕНИ шрифтове (359 КБ — Inter влиза цял,
  // защото subset-ването го поврежда) и го записва в хранилището. Пуснат
  // сам, файлът излиза за секунди. В пълния пробег обаче маршрутът се
  // компилира за пръв път ЕДВА тук, на машина, вече натоварена от
  // десетте проверки преди него.
  //
  // Разликата се вижда в резултата: сам винаги минаваше, в пълен пробег
  // падаше през път. Тоест числото беше твърде стегнато, а не кодът
  // бавен. (17.08.2026.)
  await Promise.all([
    page.waitForURL((url) => url.search.includes("izdaden=1"), { timeout: 45000 }),
    page
